...
class WSIObject():
    ...
    def __init__(self,db_entry,path_to_active_maps):
        ...
        self.level_dimension = slide.level_dimensions
        slide.close()
    ...
    def sample_whole_slide(self, crop_size, level: int = 0):
        slide_object = self.open_slidefile()
        width, height = self.level_dimension[level]
        
        imgs = []

        for x in range(0, width, round(crop_size*0.9)):
            for y in range(0, height, round(crop_size*0.9)):
                center = int((x + crop_size/2) * self.level_downsamples[level]), int((y + crop_size/2) * self.level_downsamples[level])
                is_matching = self.needs_calculation(center, crop_size, level = level)
                if is_matching:
                    imgs.append(slide_object.read_region(location = center,size = (crop_size,crop_size),level = level).convert('RGB'))
                else:
